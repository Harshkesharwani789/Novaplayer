...
describe("displayPath", () => {
  it("collapses the home prefix on windows, case-insensitively", () => {
    setPlatform("win32");
    expect(
      displayPath("C:\\Users\\Bairo\\Music\\NovaPlayer", "c:\\users\\bairo"),
    ).toBe("~\\Music\\NovaPlayer");
  });

  it("collapses the home prefix on unix, case-sensitively", () => {
    setPlatform("linux");
    expect(displayPath("/home/ale/Music/NovaPlayer", "/home/ale")).toBe(
      "~/Music/NovaPlayer",
    );
  ...
  it("passes already-tilde'd paths through", () => {
    expect(displayPath("~/Music/NovaPlayer", "/home/ale")).toBe(
      "~/Music/NovaPlayer",
    );
  });
