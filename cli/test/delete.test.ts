...
async function makeTree(): Promise<string> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "nova-delete-"));
...
describe("deleteTracks", () => {
  ...
  it("never prunes folders outside the library root", async () => {
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), "nova-out-"));
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "nova-root-"));
    try {
      const lib = new FakeLibrary();
