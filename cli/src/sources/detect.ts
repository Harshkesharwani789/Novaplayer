...
/**
 * Path segments only — the query string and fragment are cut off first.
 *
 * Splitting on `[/?#]` in one pass (the previous behaviour) turned every
 * `?key=value` into an extra "segment", which silently changed what a link was
 * classified as: `soundcloud.com/artist` is a profile, but the exact same link
 * with the tracking param SoundCloud's own Share button appends —
 * `soundcloud.com/artist?si=…` — parsed as two segments and was classified as
 * a single *track*. Downloading it then enumerated and enqueued the artist's
 * entire profile, which is precisely the accident the profile/track split
 * exists to prevent.
 */
