...
import { primeBrowserDetection } from "../bin/browsers";
...
export function App({ initialAdd }: { initialAdd?: string } = {}) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      // Apply the saved cookie setting before anything can spawn yt-dlp, and
      // warm the browser scan so the Download banner can name a real browser
      // without awaiting inside a render.
      setCookieBrowser(cfg.cookiesFromBrowser);
      void primeBrowserDetection().catch(() => {});
      ...
      const history = await PlayHistory.load(library.database);
      ...
      playback.on("state", (s: PlaybackState) => {
        ...
        if (id && id !== lastPlayedId) {
          lastPlayedId = id;
          history.record(id, s.track?.durationSec ?? 0);
        }
      });
      ...
      // Downloads hard-gate on yt-dlp + ffmpeg (both fetched in the background
      // by ensureBinaries, and re-awaited here since either may still be
      // in flight); undefined keeps the default concurrency.
  ...
  const store: Store | null = useMemo(() => {
    ...
    return {
      ...
      region: showHelp || showPalette ? "help" : region,
      setRegion,
      captureMode,
      setCaptureMode,
      playlistsDepth,
  ...
  if (error) {
    // yt-dlp/ffmpeg no longer block boot (they're fetched in the background
    // and only gate an actual download), so a failure here is something local
    // — a corrupt config, a missing/unwritable data directory, a library load
    // error — not "no internet". Show the real reason instead of guessing.
    return (
      <Box flexDirection="column" paddingX={1} paddingY={1}>
        <Text color={COLOR.warn}>Couldn&apos;t finish setup</Text>
        <Box marginTop={1}>
          <Text dimColor wrap="wrap">{error}</Text>
        </Box>
        <Box marginTop={1}>
  ...
  const paletteCommands: PaletteCommand[] = [
    {
      id: "play-song",
      label: "Play song",
      detail: "Play the newest track in your library",
      run: () => {
        const track = boot?.library.all()[0];
        if (track) playTrack(track, boot?.library.all());
      },
    },
    {
      id: "search-library",
      label: "Search library",
      detail: "Open ranked fuzzy search",
      run: () => { setSection("library"); setRegion("content"); setPendingSearch(true); },
    },
    {
      id: "create-playlist",
      label: "Create playlist",
      detail: "Create an empty playlist in SQLite",
      run: () => {
        try {
          boot?.library.createPlaylist(`New playlist ${new Date().toLocaleDateString()}`);
          setPaletteStatus("Created a new playlist");
          setSection("playlists");
          setRegion("content");
        } catch (error) {
          setPaletteStatus(error instanceof Error ? error.message : "Couldn't create playlist");
        }
      },
    },
    {
      id: "scan-library",
      label: "Scan library",
      detail: "Index local audio files incrementally",
      run: () => {
        if (!boot || !config) return;
        setPaletteStatus("Scanning local library…");
        void scanLibrary(config.libraryDir, boot.library, (progress) => {
          setPaletteStatus(`Scanning ${progress.scanned} files · ${progress.indexed} indexed`);
        }).then((result) => setPaletteStatus(`Scan complete · ${result.indexed} indexed, ${result.duplicates} duplicates`))
          .catch((error: unknown) => setPaletteStatus(error instanceof Error ? error.message : "Scan failed"));
      },
    },
    {
      id: "analytics",
      label: "Open analytics",
      detail: "Review local listening trends",
      run: () => { setSection("analytics"); setRegion("content"); },
    },
    {
      id: "queue-controls",
      label: "Queue controls",
      detail: "Toggle shuffle for the current queue",
      run: () => { boot?.playback.toggleShuffle(); setPaletteStatus("Queue shuffle toggled"); },
    },
    {
      id: "settings",
      label: "Settings",
      detail: "Configure NovaPlayer",
      run: () => { setSection("settings"); setRegion("content"); },
    },
  ];
      ...
      <Box flexDirection="column" paddingX={1}>
        ...
        {welcome ? (
          ...
          <>
            ...
            {paletteStatus ? (
              <Box marginTop={1} display={showHelp || showPalette ? "none" : "flex"}>
                <Text dimColor>{paletteStatus}</Text>
              </Box>
            ) : null}
            <Box
              height={bodyH}
              marginTop={1}
              display={showHelp || showPalette ? "none" : "flex"}
            >
              <Sidebar />
              <Box
                flexGrow={1}
