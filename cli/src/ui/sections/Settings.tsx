...
import { nextBrowser, installedBrowsersSync } from "../../bin/browsers";
...
export function Settings() {
  ...
  const entries: {
    value: Mode | "open-folder" | "cookies";
    name: string;
    detail: string;
    set?: boolean;
    danger?: boolean;
    ...
    {
      // A cycling toggle rather than a sub-page: the value is one of a short
      // fixed list, so pressing enter to advance it is fewer keystrokes than
      // opening a picker, and the current value stays readable in the row.
      value: "cookies",
      name: "Browser cookies",
      detail: config.cookiesFromBrowser
        ? `${config.cookiesFromBrowser} · ↵ changes`
        : installedBrowsersSync().length > 0
          ? `off · ↵ for ${installedBrowsersSync().slice(0, 3).join(", ")}`
          : 'off · ↵ if YouTube says "not a bot"',
  ...
  function openSetting(v: Mode | "open-folder" | "cookies"): void {
    if (v === "cookies") {
      // Cycles only through browsers detected on this machine, so the setting
      // can't land on one the user doesn't have.
      const nextBrowser_ = nextBrowser(config.cookiesFromBrowser);
      // Apply to the live yt-dlp invocation path immediately as well as
      // persisting, so the very next retry uses the new setting without a
      // restart — the whole point is that the user is mid-failure right now.
      applyCookieBrowser(nextBrowser_);
      setConfig({ ...config, cookiesFromBrowser: nextBrowser_ });
      return;
    }
    if (v === "open-folder") {
        ...
        <Select
          ...
          onChange={(v) => {
            ...
            void (async () => {
              ...
              await library.clear();
              // Remove the folders NovaPlayer creates (catches completed files,
