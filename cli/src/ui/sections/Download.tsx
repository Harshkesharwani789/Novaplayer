...
import { suggestedBrowser } from "../../bin/browsers";
import { setCookieBrowser as applyCookieBrowser } from "../../bin/binaries";
import { WAITING_FOR_TOOLS, type QueueItem } from "../../download/queue";
import type { SourceAdapter, SourcePlaylist } from "../../sources/types";
import { SOURCE_LABELS, type RemoteSourceId, type SourceId } from "../../library/types";
...
function shortError(e?: string): string {
  ...
  // Auth gates come before the generic 403 bucket on purpose. A 403 from
  // YouTube is almost always its bot check, and "retry later" — what this used
  // to say — is advice that can never work: the request is missing a signed-in
  // session, not waiting out a limit, so every retry fails identically. Point
  // at the setting that actually fixes it instead.
  //
  // Each needle is anchored to a phrase rather than a fragment. A bare "age"
  // also matches "message", "usage" and "package"; a bare "login" matches
  // "logout". Those false positives are worse than a miss here, because they
  // send someone to configure cookies for a failure cookies cannot fix.
  if (
    m.includes("browser cookies") ||
    m.includes("sign in") ||
    ...
    return "YouTube refused · try cookies";
  if (
...
function QueueView() {
  const { queue, region, listRows, config, setConfig } = useStore();
  const items = useQueueItems(queue);
  const s = queue.stats();
  const focused = region === "content";
  const [offset, setOffset] = useState(0);

  // A whole playlist failing the same way is a configuration problem, not 17
  // unlucky tracks. Counting them lets the UI say so once, loudly, with the
  // actual keystroke — instead of repeating a cryptic hint on every red row
  // and leaving the user to infer that they all share one cause and one fix.
  const authGateFailures = items.filter(
    (i) => i.status === "error" && shortError(i.error).startsWith("YouTube refused"),
  ).length;
  const showAuthGateHelp = authGateFailures >= 2;
  // Offer the fix with the browser this machine actually has, so the banner can
  // name it ("b  use Chrome cookies") instead of sending the user off to a
  // settings screen to guess from a list that includes browsers they lack.
  const suggested = suggestedBrowser();
  const canOfferCookies = showAuthGateHelp && !config.cookiesFromBrowser && Boolean(suggested);

  /** Turn cookies on with the detected browser and immediately retry. */
  const enableCookiesAndRetry = (): void => {
    if (!suggested) return;
    applyCookieBrowser(suggested);
    setConfig({ ...config, cookiesFromBrowser: suggested });
    queue.retryFailed();
  };
  ...
  useInput(
    (input, key) => {
      if (input === "[") {
        ...
        // The fix, from where the failure is: no navigating to Settings, no
        // choosing from a list of browsers this machine may not even have.
        if (canOfferCookies) enableCookiesAndRetry();
      } else if (key.upArrow) {
  ...
  if (canOfferCookies) cmds.push({ key: "b", label: `Use ${suggested} cookies` });
  ...
  return (
    <Box flexDirection="column">
      {s.rateLimited ? (
          ...
          <Text color={COLOR.warn} wrap="truncate-end">
            {`${ICON.warn} ${
              s.rateLimitReason === WAITING_FOR_TOOLS
                ? "Waiting for the downloader (yt-dlp/ffmpeg — check your internet)"
        ...
        <Box marginBottom={1}>
          <Text color={COLOR.warn} wrap="truncate-end">
            {`${ICON.warn} ${s.failingSource} downloads keep failing  ${ICON.dot}  the downloader may be out of date  ${ICON.dot}  restart NovaPlayer to update it`}
          </Text>
        </Box>
      ) : null}
      ...
      {showAuthGateHelp ? (
        // The one banner that has to be unmissable: it names the cause, the
        // exact fix, and (when cookies are already on) the next thing to try.
        <Box flexDirection="column" marginBottom={1}>
          <Text color={COLOR.warn} wrap="truncate-end">
            {`${ICON.warn} YouTube refused ${authGateFailures} tracks. NovaPlayer already retried each one as a TV, Safari and iOS client.`}
          </Text>
          {canOfferCookies ? (
            <Text wrap="truncate-end">
              <Text color={COLOR.accent}>b</Text>
              <Text dimColor>{` use your ${suggested} cookies and retry`}</Text>
              <Text dimColor>{`  ${ICON.dot}  macOS may ask once for Keychain access`}</Text>
            </Text>
          ) : config.cookiesFromBrowser ? (
            <Text dimColor wrap="truncate-end">
              {`Cookies are on (${config.cookiesFromBrowser}). Sign in to YouTube in ${config.cookiesFromBrowser}, fully quit it, then press f to retry.`}
            </Text>
          ) : (
            <Text dimColor wrap="truncate-end">
              Fix: Settings → Browser cookies → ↵ to pick your browser, then f to retry.
            </Text>
          )}
          <Text dimColor wrap="truncate-end">
            {`Waiting will not help ${ICON.dot} full errors: ${displayPath(downloadLogFile)}`}
          </Text>
        </Box>
      ) : null}
...
export function Download() {
  ...
  if (step.name === "need-handle") {
    ...
    const p = PROMPTS[step.source as RemoteSourceId];
    return (
      <Box flexDirection="column">
